(() => {
  ...
  const URLSP = g.URLSearchParams;
  if (!URLSP) return;
  const getQuery = () => new URLSP(g.location ? g.location.search : '');
  const applyFromUrl = () => {
    const qs = getQuery();
  ...
  const renderLinks = (links) => {
    ...
    const html = links
      .map((l) => {
        const href0 = linkHrefMap && linkHrefMap[l] ? linkHrefMap[l] : l;
        const href = escapeHtml(href0);
