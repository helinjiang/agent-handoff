...
AgentHandoff 的目标：通过"**线性步骤链（Step Pipeline）+ 产物驱动（Artifact-first）**"实现多 Agent 协作：上一步骤产物交给下一步骤作为输入，在上下文隔离前提下尽可能共享关键信息。
...
命令建议：
- `agent-handoff init <name>`：生成 workspace 模板（workflow.yaml + steps 目录）
...
AgentHandoff 关注"多 Agent 协作与接力编排（workflow/steps）"。  
